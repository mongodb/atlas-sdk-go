...
type CloudProviderContainer struct {
	...
	AtlasCidrBlock *string `json:"atlasCidrBlock,omitempty" validate:"regexp=^((([0-9]{1,3}\\\\.){3}[0-9]{1,3})|(:{0,2}([0-9a-f]{1,4}:){0,7}[0-9a-f]{1,4}[:]{0,2}))((%2[fF]|\\/)[0-9]{1,3})+$"`
	// Unique string that identifies the Azure subscription in which the MongoDB Cloud VNet resides.
	// Read only field.
	AzureSubscriptionId *string `json:"azureSubscriptionId,omitempty" validate:"regexp=^([a-f0-9]{24})$"`
	...
	// Read only field.
	NetworkName *string `json:"networkName,omitempty" validate:"regexp=^nt-[0-9a-f]{24}-[0-9a-z]{8}$"`
