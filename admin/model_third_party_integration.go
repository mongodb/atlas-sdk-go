...
type ThirdPartyIntegration struct {
	...
	// Two-letter code that indicates which regional URL MongoDB uses to access the Datadog API.  Alternatively: Two-letter code that indicates which regional URL MongoDB uses to access the Opsgenie API.  Alternatively: PagerDuty region that indicates the API Uniform Resource Locator (URL) to use.
...
func (o ThirdPartyIntegration) ToMap() (map[string]interface{}, error) {
	...
	if !IsNil(o.SendCollectionLatencyMetrics) {
		toSerialize["sendCollectionLatencyMetrics"] = o.SendCollectionLatencyMetrics
	}
	if !IsNil(o.SendDatabaseMetrics) {
		toSerialize["sendDatabaseMetrics"] = o.SendDatabaseMetrics
	}
