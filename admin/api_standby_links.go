...
type StandbyLinksAPI interface {
	...
	/*
		 ...
		 Creates a disaster recovery standby link between an active cluster and a standby cluster. Both clusters must not already be part of a standby link system and must be single-region clusters.
	...
	*/
	...
	/*
			InspectFailoverReadiness Inspect Failover Readiness for One Standby Link

			This API is in preview. Breaking changes might be introduced before it is released. Don't use preview APIs in production.

		 Returns the failover readiness metrics for the specified standby link in the specified project. The response includes the readiness snapshot identifiers, estimated data loss and failover time values. Use this endpoint to assess potential data loss before triggering an unplanned failover.

			@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
			@param groupId Unique 24-hexadecimal digit string that identifies the project.
			@param standbyLinkId Unique 24-hexadecimal digit string that identifies the standby link.
			@return InspectFailoverReadinessApiRequest
	*/
	InspectFailoverReadiness(ctx context.Context, groupId string, standbyLinkId string) InspectFailoverReadinessApiRequest
	/*
		InspectFailoverReadiness Inspect Failover Readiness for One Standby Link


		@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
		@param InspectFailoverReadinessApiParams - Parameters for the request
		@return InspectFailoverReadinessApiRequest
	*/
	InspectFailoverReadinessWithParams(ctx context.Context, args *InspectFailoverReadinessApiParams) InspectFailoverReadinessApiRequest

	// Method available only for mocking purposes
	InspectFailoverReadinessExecute(r InspectFailoverReadinessApiRequest) (*ApiStandbyLinkFailoverReadinessResponse, *http.Response, error)
...
func (a *StandbyLinksAPIService) CreateStandbyLinkFailoverExecute(r CreateStandbyLinkFailoverApiRequest) (*ApiStandbyLinkFailoverResponse, *http.Response, error) {
	...
	if r.standbyLinkId == "." || r.standbyLinkId == ".." {
		return localVarReturnValue, nil, reportError("standbyLinkId must not be a dot-segment path parameter")
	}
...
func (a *StandbyLinksAPIService) DeleteGroupStandbyLinkExecute(r DeleteGroupStandbyLinkApiRequest) (*http.Response, error) {
	...
	if r.standbyLinkId == "." || r.standbyLinkId == ".." {
		return nil, reportError("standbyLinkId must not be a dot-segment path parameter")
	}
	localVarPath = strings.Replace(localVarPath, "{"+"standbyLinkId"+"}", url.PathEscape(r.standbyLinkId), -1)
...
func (a *StandbyLinksAPIService) GetStandbyLinkFailoverExecute(r GetStandbyLinkFailoverApiRequest) (*ApiStandbyLinkFailoverResponse, *http.Response, error) {
	...
	if r.failoverId == "." || r.failoverId == ".." {
		return localVarReturnValue, nil, reportError("failoverId must not be a dot-segment path parameter")
	}
...
type InspectFailoverReadinessApiRequest struct {
	ctx           context.Context
	ApiService    StandbyLinksAPI
	groupId       string
	standbyLinkId string
...
type InspectFailoverReadinessApiParams struct {
	GroupId       string
	StandbyLinkId string
	Envelope      *bool
}

func (a *StandbyLinksAPIService) InspectFailoverReadinessWithParams(ctx context.Context, args *InspectFailoverReadinessApiParams) InspectFailoverReadinessApiRequest {
	return InspectFailoverReadinessApiRequest{
		ApiService:    a,
		ctx:           ctx,
		groupId:       args.GroupId,
...
// Flag that indicates whether to wrap the response in an envelope.
func (r InspectFailoverReadinessApiRequest) Envelope(envelope bool) InspectFailoverReadinessApiRequest {
	r.envelope = &envelope
	return r
}

func (r InspectFailoverReadinessApiRequest) Execute() (*ApiStandbyLinkFailoverReadinessResponse, *http.Response, error) {
	return r.ApiService.InspectFailoverReadinessExecute(r)
}
...
This API is in preview. Breaking changes might be introduced before it is released. Don't use preview APIs in production.
	...
	@return InspectFailoverReadinessApiRequest
*/
func (a *StandbyLinksAPIService) InspectFailoverReadiness(ctx context.Context, groupId string, standbyLinkId string) InspectFailoverReadinessApiRequest {
	return InspectFailoverReadinessApiRequest{
		ApiService:    a,
		ctx:           ctx,
		groupId:       groupId,
...
// InspectFailoverReadinessExecute executes the request
//
//	@return ApiStandbyLinkFailoverReadinessResponse
func (a *StandbyLinksAPIService) InspectFailoverReadinessExecute(r InspectFailoverReadinessApiRequest) (*ApiStandbyLinkFailoverReadinessResponse, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    any
		formFiles           []formFile
		localVarReturnValue *ApiStandbyLinkFailoverReadinessResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "StandbyLinksAPIService.InspectFailoverReadiness")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/atlas/v2/groups/{groupId}/standbyLinks/{standbyLinkId}:inspectFailoverReadiness"
	if r.groupId == "" {
		return localVarReturnValue, nil, reportError("groupId is empty and must be specified")
	}
	if r.groupId == "." || r.groupId == ".." {
