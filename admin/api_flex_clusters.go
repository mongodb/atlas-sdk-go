...
type FlexClustersAPI interface {
	...
	*/
	...
	/*
			EstimateUpgradeFlexCost Estimate the Cost of Upgrading One Flex Cluster
		 ...
		 This endpoint validates a supplied cluster configuration and returns an estimated hourly and monthly compute cost without creating or modifying the cluster. It applies the same validation as the Upgrade One Flex Cluster endpoint, so invalid configurations are rejected with the same errors. The estimate excludes backup storage, disk storage, data transfer, and premium feature surcharges, which are billed based on actual usage. Subscription, payment, and resource policy checks are not performed, so a configuration that can be priced here may still be rejected during the actual operation.
			...
			@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
			@param groupId Unique 24-hexadecimal digit string that identifies your project. Use the [/groups](#tag/Projects/operation/listProjects) endpoint to retrieve all projects to which the authenticated user has access.  **NOTE**: Groups and projects are synonymous terms. Your group id is the same as your project id. For existing groups, your group/project id remains the same. The resource and corresponding endpoints use the term groups.
			@param atlasTenantClusterUpgradeRequest20240805 Flex cluster upgrade to estimate.
			@return EstimateUpgradeFlexCostApiRequest
	*/
	EstimateUpgradeFlexCost(ctx context.Context, groupId string, atlasTenantClusterUpgradeRequest20240805 *AtlasTenantClusterUpgradeRequest20240805) EstimateUpgradeFlexCostApiRequest
	/*
		EstimateUpgradeFlexCost Estimate the Cost of Upgrading One Flex Cluster


		@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
		@param EstimateUpgradeFlexCostApiParams - Parameters for the request
		@return EstimateUpgradeFlexCostApiRequest
	*/
	EstimateUpgradeFlexCostWithParams(ctx context.Context, args *EstimateUpgradeFlexCostApiParams) EstimateUpgradeFlexCostApiRequest

	// Method available only for mocking purposes
	EstimateUpgradeFlexCostExecute(r EstimateUpgradeFlexCostApiRequest) (*ClusterCostEstimate, *http.Response, error)
...
type EstimateUpgradeFlexCostApiRequest struct {
	ctx                                      context.Context
	ApiService                               FlexClustersAPI
	groupId                                  string
...
type EstimateUpgradeFlexCostApiParams struct {
	GroupId                                  string
	AtlasTenantClusterUpgradeRequest20240805 *AtlasTenantClusterUpgradeRequest20240805
}

func (a *FlexClustersAPIService) EstimateUpgradeFlexCostWithParams(ctx context.Context, args *EstimateUpgradeFlexCostApiParams) EstimateUpgradeFlexCostApiRequest {
	return EstimateUpgradeFlexCostApiRequest{
		ApiService:                               a,
		ctx:                                      ctx,
		groupId:                                  args.GroupId,
...
func (r EstimateUpgradeFlexCostApiRequest) Execute() (*ClusterCostEstimate, *http.Response, error) {
	return r.ApiService.EstimateUpgradeFlexCostExecute(r)
}
...
This API is in preview. Breaking changes might be introduced before it is released. Don't use preview APIs in production.
	...
	@return EstimateUpgradeFlexCostApiRequest
*/
func (a *FlexClustersAPIService) EstimateUpgradeFlexCost(ctx context.Context, groupId string, atlasTenantClusterUpgradeRequest20240805 *AtlasTenantClusterUpgradeRequest20240805) EstimateUpgradeFlexCostApiRequest {
	return EstimateUpgradeFlexCostApiRequest{
		ApiService:                               a,
		ctx:                                      ctx,
		groupId:                                  groupId,
...
// EstimateUpgradeFlexCostExecute executes the request
//
//	@return ClusterCostEstimate
func (a *FlexClustersAPIService) EstimateUpgradeFlexCostExecute(r EstimateUpgradeFlexCostApiRequest) (*ClusterCostEstimate, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    any
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FlexClustersAPIService.EstimateUpgradeFlexCost")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/atlas/v2/groups/{groupId}/flexClusters:estimateTenantUpgradeCost"
	if r.groupId == "" {
		return localVarReturnValue, nil, reportError("groupId is empty and must be specified")
	}
	localVarPath = strings.Replace(localVarPath, "{"+"groupId"+"}", url.PathEscape(r.groupId), -1)
