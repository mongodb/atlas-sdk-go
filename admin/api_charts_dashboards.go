...
type ChartsDashboardsAPI interface {
	...
	/*
		 ...
		 Exports the specified Charts dashboard.
	...
	*/
	...
	/*
		 ...
		 Imports the Charts dashboard that the template specifies. Optionally, you can specify `overwrite=true` to import into an existing dashboard.
...
func (a *ChartsDashboardsAPIService) ExportChartsDashboardExecute(r ExportChartsDashboardApiRequest) (string, *http.Response, error) {
	...
	if r.dashboardId == "." || r.dashboardId == ".." {
		return localVarReturnValue, nil, reportError("dashboardId must not be a dot-segment path parameter")
	}
