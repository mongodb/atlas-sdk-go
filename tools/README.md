...
### Transformer CLI usage

The transformer is also available as a CLI for local use:

```bash
# Full transform (in-place)
node ./transformer/bin/atlas-openapi-transformer.js transform -i ../openapi/atlas-api-transformed.yaml

# Flatten-only, reading from stdin and writing to stdout
cat ../openapi/atlas-api.yaml | node ./transformer/bin/atlas-openapi-transformer.js flatten --stdin --stdout > ../openapi/atlas-api-flattened.yaml

# Adjust log level
node ./transformer/bin/atlas-openapi-transformer.js transform -i ../openapi/atlas-api-transformed.yaml --log-level info
```
